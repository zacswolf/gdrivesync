...
async function promptForSecretInput(prompt: string): Promise<string> {
  if (!process.stdin.isTTY) {
    ...
    const pipedValue = Buffer.concat(chunks).toString("utf8").trim();
    if (!pipedValue) {
...
async function promptForChoice<T>(
  ...
  try {
    while (true) {
      ...
      const answer = (await rl.question("Choose an option (Enter to cancel): ")).trim();
      if (!answer) {
        return undefined;
      }
...
async function runCliImageEnrichmentWizard(
  ...
  if (!process.stdin.isTTY) {
    throw new Error("gdrivesync configure image-enrichment requires an interactive terminal.");
  }
